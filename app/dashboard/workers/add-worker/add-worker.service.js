...
var AddWorkerService = (function () {
    function AddWorkerService(http, config, authService) {
        this.http = http;
        this.config = config;
        this.authService = authService;
        this.addWorkerUrl = this.config.getConfig().apiMainUrl + "/users/create"; // URL to web api
        this.headers = new http_1.Headers({ 'Content-Type': 'application/json', 'authorization': this.authService.getAuthorizationHeader() });
    }
    AddWorkerService.prototype.addWorker = function (user, currentCompany) {

...
var WorkerEditService = (function () {
    function WorkerEditService(http, config, authService) {
        this.http = http;
        this.config = config;
        this.authService = authService;
        this.WorkerEditUrl = this.config.getConfig().apiMainUrl + "/users/"; // URL to web api
        this.headers = new http_1.Headers({ 'Content-Type': 'application/json', 'authorization': this.authService.getAuthorizationHeader() });
    }
    WorkerEditService.prototype.updateWorker = function (user, companyId, userId) {
        console.log(user);
